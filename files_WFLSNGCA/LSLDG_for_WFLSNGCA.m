function [g,nabla_g,theta,hparams,MedDim,C,cind]=LSLDG_for_WFLSNGCA(X,op,C,MedDim)
%
...
% Output:
%   g : log-density gradients (op.dim*op.samples)
%   nabla_g : derivatives of log-density gradients (op.dim*op.dim*op.samples)
%              (1ŽŸŒ³–Ú‚Æ‚QŽŸŒ³–Ú‚Ì‡”Ô‚É’ˆÓ)
%   theta : learned parameters (op.bnum*op.dim)
...
%         sigma_tmp=op.sigma_list(sigma_index);
        ...
        for kk=cv_fold  
            ...
            elseif op.bfunc == 2
                psi_train = [XC_diff(1:b2,cv_index~=kk,dd).*GauKer(1:b2,cv_index~=kk);...
                                X_XC_diff(b2+1:op.bnum,cv_index~=kk)]; %b*n
                psi_test = [XC_diff(1:b2,cv_index==kk,dd).*GauKer(1:b2,cv_index==kk);...
                                X_XC_diff(b2+1:op.bnum,cv_index==kk)]; %b*n %b*n
                phi_train = [(XC_diff(1:b2,cv_index~=kk,dd).^2/sigma_tmp^2-1).*GauKer(1:b2,cv_index~=kk);...
                             XC_diff(b2+1:op.bnum,cv_index~=kk,dd)-X_repmat(b2+1:op.bnum,cv_index~=kk,dd)];%b*n
                phi_test = [(XC_diff(1:b2,cv_index==kk,dd).^2/sigma_tmp^2-1).*GauKer(1:b2,cv_index==kk);...
                             XC_diff(b2+1:op.bnum,cv_index==kk,dd)-X_repmat(b2+1:op.bnum,cv_index==kk,dd)];%b*n
            else
...
for dd=1:op.dim
    ...
    elseif op.bfunc == 2
        psi = [XC_diff(1:b2,:,dd).*GauKer(1:b2,:);...
                X_XC_diff(b2+1:op.bnum,:)]; %b*n
        phi = [(XC_diff(1:b2,:,dd).^2/hparams.sigma(dd)^2-1).*GauKer(1:b2,:);...
                XC_diff(b2+1:op.bnum,:,dd)-X_repmat(b2+1:op.bnum,:,dd)];%b*n
    else
        psi=GauKer;
...
%% compute grad 
GauKer3D=exp(-bsxfun(@times,XC_dist,1./(2*permute(hparams.sigma.^2,[2,3,1])))); % b*n*d

if op.bfunc == 1
    psi_g=XC_diff.*GauKer3D;
    phi_tmp = bsxfun(@rdivide,XC_diff,permute(hparams.sigma.^2,[2,3,1])); % b*n*d
    phi_g = bsxfun(@times,bsxfun(@minus,bsxfun(@times,phi_tmp,permute(XC_diff,[1,2,4,3])),permute(eye(op.dim),[3,4,1,2])),GauKer3D);%b*n*d*d
elseif op.bfunc == 2
    psi_g = [XC_diff(1:b2,:,:).*GauKer3D(1:b2,:,:);...
                repmat(X_XC_diff(b2+1:op.bnum,:),[1,1,op.dim])]; %b*n*d
    phi_tmp = bsxfun(@rdivide,XC_diff(1:b2,:,:),permute(hparams.sigma.^2,[2,3,1])); % b*n*d
    phi_g = [bsxfun(@times,bsxfun(@minus,bsxfun(@times,phi_tmp,permute(XC_diff(1:b2,:,:),[1,2,4,3])),permute(eye(op.dim),[3,4,1,2])),GauKer3D(1:b2,:,:));%b*n*d*d
                repmat(XC_diff(b2+1:op.bnum,:,:)-X_repmat(b2+1:op.bnum,:,:),[1,1,1,op.dim])];%b*n
...
nabla_g = permute(sum(bsxfun(@times,theta,permute(phi_g,[1,3,4,2])),1),[2,3,4,1]);%d*d*n
